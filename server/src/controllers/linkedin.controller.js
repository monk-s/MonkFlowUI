...
const listLeads = catchAsync(async (req, res) => {
  ...
  const params = [limit];
  let sql = `SELECT id, business_name, contact_name, contact_title, business_city, status, score, connect_sent_at, connected_at, dm_sent_at, replied_at, linkedin_url, last_touch_at FROM linkedin_leads`;
  if (status) {
    sql += ` WHERE status = $2`;
    params.push(status);
  }
